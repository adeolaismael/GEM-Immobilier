...
function BiensGridCardItem({
  ...
  const indisponible = isBienIndisponible(bien.statut);
  ...
  return (
    <motion.article
      ...
      className={`group rounded-2xl bg-[color:var(--card)] p-6 ring-1 ring-[color:var(--card-border)] transition-all duration-300 ${
        indisponible
          ? "cursor-not-allowed opacity-75"
          : "hover:-translate-y-1 hover:shadow-lg"
      }`}
      onMouseEnter={() => !indisponible && setIsHovered(true)}
      onMouseLeave={() => !indisponible && setIsHovered(false)}
    >
