...
import { CONTACT_SERVICES } from "@/lib/contact-constants";
...
const selectClassName =
  "mt-2 h-11 w-full rounded-xl bg-[color:var(--card)] px-4 text-sm text-foreground ring-1 ring-[color:var(--card-border)] focus:outline-none focus:ring-2 focus:ring-[color:var(--brand)]/60";
...
function ContactFormInner({
  ...
  const decodedTitre = titre ? decodeURIComponent(titre) : "";
  const serviceConcernLabel = decodedTitre || decodedService;
  ...
  useEffect(() => {
    ...
    if (serviceParam) {
      const label = decodeURIComponent(serviceParam);
      if ((CONTACT_SERVICES as readonly string[]).includes(label)) {
        setService(label);
      }
      const subjectTitle = titre ? decodeURIComponent(titre) : label;
      setSujet(`Demande d'information - ${subjectTitle}`);
    }
  }, [bienId, titre, serviceParam]);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const serviceVal =
      showServiceSelect || serviceParam
        ? ((formData.get("service") as string) || service || decodedService || null)
        : null;
  ...
  return (
    <section className="rounded-2xl bg-white/5 p-6 ring-1 ring-white/10">
      ...
      {bienId && titre && (
        <div className="mt-4 rounded-xl bg-[color:var(--brand)]/10 px-4 py-3 ring-1 ring-[color:var(--brand)]/20">
          <p className="text-sm font-medium text-[color:var(--foreground)]">
            ...
            <span className="font-semibold">{decodeURIComponent(titre)}</span>
          </p>
        </div>
      )}

      {serviceParam && !bienId && serviceConcernLabel && (
        <div className="mt-4 rounded-xl bg-[color:var(--brand)]/10 px-4 py-3 ring-1 ring-[color:var(--brand)]/20">
          <p className="text-sm font-medium text-[color:var(--foreground)]">
            {texts.form_concern_prefix}{" "}
            <span className="font-semibold">{serviceConcernLabel}</span>
          </p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        {showServiceSelect && (
          <label className="block">
            <span className="text-sm text-[color:var(--muted)]">{texts.label_service}</span>
            <select
